...
//卡尔曼滤波初值
double pre_ball_distance = 100;
double pre_kf_ball_distance = 100;

// 平均值队列
vector<float> pre_ball_distance_vec;
...
void CreateBaseCalculatePositionMsg(calculate_position_pkg::calculate_position_result &cal_pos_res, const calculate_position_pkg::image_points::ConstPtr& msg) {
	cal_pos_res.ball_xyxy = msg->ball_xyxy;
	cal_pos_res.goalpost_xyxy = msg->goalpost_xyxy;
	cal_pos_res.robot_xyxy = msg->robot_xyxy;
	cal_pos_res.L_mark_xyxy = msg->L_mark_xyxy;
	cal_pos_res.T_mark_xyxy = msg->T_mark_xyxy;
	cal_pos_res.X_mark_xyxy = msg->X_mark_xyxy;
}

// // 构造辅助请求包
// void createUdpDate(UdpData& data ,calculate_position_pkg::calculate_position_result cal_pos_res) {
// 	if(cal_pos_res.ball_xyxy.size() == 0) {
// 		for(int i = 0; i < 4; i++) {
// 			data.ball_xyxy[i] = 0;
// 		}
// 	}else {
// 		for(int i = 0; i < cal_pos_res.ball_xyxy.size(); i++) {
// 			data.ball_xyxy[i] = cal_pos_res.ball_xyxy[i];
// 		}
// 	}

// 	if(cal_pos_res.goalpost_xyxy.size() == 0) {
// 		for(int i = 0; i < 4; i++) {
// 			data.goalpost_xyxy[i] = 0;
// 		}
// 	}else {
// 		for(int i = 0; i < cal_pos_res.goalpost_xyxy.size(); i++) {
// 			data.goalpost_xyxy[i] = cal_pos_res.goalpost_xyxy[i];
// 		}
// 	}
...
// 	if(cal_pos_res.X_mark_xyxy.size() == 0) {
// 		for(int i = 0; i < 4; i++) {
// 			data.X_mark_xyxy[i] = 0;
// 		}
// 	}else {
// 		for(int i = 0; i < cal_pos_res.X_mark_xyxy.size(); i++) {
// 			data.X_mark_xyxy[i] = cal_pos_res.X_mark_xyxy[i];
// 		}
// 	}
...
float cal_distance(std::vector<Point2d> image_points, std::vector<Point3d> model_points) {
	...
	*/
	...
	// Z轴
	float ball_distance = P_oc.at<float>(2,0);
...
/* 
	...
	PNP计算球
*/
void CalculateDis(const calculate_position_pkg::image_points::ConstPtr& msg) {
	// 限制
	if(msg -> ball_xyxy.size() == 0) {
		...
		cal_pos_res.ball_distance = pre_ball_distance;
		cal_pos_res.kf_ball_distance = pre_kf_ball_distance;
		pub_cal_pos_res.publish(cal_pos_res);
		cout << "cal pub data" <<endl;
		return;
	}
	...
	*/
	...
	// 计算球点
	int football_point1_x = msg->ball_xyxy[0];
	int football_point1_y = msg->ball_xyxy[1];
	int football_point2_x = msg->ball_xyxy[2];
	int football_point2_y = msg->ball_xyxy[3];
	...
	int football_point5_x = msg->ball_xyxy[0] + (msg->ball_xyxy[2]-msg->ball_xyxy[0])/4;
	...
	//计算足球距离
	float ball_distance = cal_distance(image_points, model_points);
	...
	// 卡尔曼滤波
	float kf_ball_distance = kf -> get_my_kalman_filter_result(ball_distance);
	cout << "ball_distance: " << ball_distance << endl;
	cout << "kf_football_distance: " << kf_ball_distance << endl;

	// 迭代卡尔曼滤波值
	pre_ball_distance = ball_distance;
	pre_kf_ball_distance = kf_ball_distance;

	// 构造发布msg
	calculate_position_pkg::calculate_position_result cal_pos_res;
	cal_pos_res.ball_distance = ball_distance;
	// 4v4 distance
	// cal_pos_res.kf_ball_distance = kf_ball_distance;

	// 计算10次平均值
	if(pre_ball_distance_vec.size() >= 10) {
		pre_ball_distance_vec.push_back(kf_ball_distance);
		pre_ball_distance_vec.erase(pre_ball_distance_vec.begin());
	}else {
		pre_ball_distance_vec.push_back(kf_ball_distance);
	}
	// 求平均
	float average = calAverage(pre_ball_distance_vec);
	cout << "pre 10 average: " << average << endl;
	// 求中位
	vector<float> pre_ball_distance_vec_sort = pre_ball_distance_vec;
	sort(pre_ball_distance_vec_sort.begin(), pre_ball_distance_vec_sort.end());
	if(pre_ball_distance_vec_sort.size() >= 10) {
		cout << "pre 10 mid: " << pre_ball_distance_vec_sort[5] << endl;
	}

	cal_pos_res.kf_ball_distance = average;
	CreateBaseCalculatePositionMsg(cal_pos_res, msg);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = nh.subscribe("chatter_image_points", 100, CalculateDis);
