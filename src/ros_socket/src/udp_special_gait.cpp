...
int main(int argc, char** argv)
{
    ...
    servaddr.sin_port = htons(1111);//��һ���޷��Ŷ����͵�������ֵת��Ϊ�����ֽ�˳�򣬼���β˳��(big-endian)
    servaddr.sin_addr.s_addr = inet_addr(local_ip_addr.data());//net_addr��������ת���ַ�������Ҫ������һ��ʮ���Ƶ���ת��Ϊ�����Ƶ�������;����ipv4��IPת����
    bind(serv_socket,(struct sockaddr*)&servaddr,sizeof(servaddr));
    if (serv_socket == -1)
    {
        cout << "udp socket  init failed"<< endl;
        exit(1);
    }
    cout << "udp socket init succcess\n" << endl;
            ...
            if(n!=0 && n!=-1){
                ...
                if(token_str == "special_gait_data"){
                    ...
                    for (int i = 1; i <= 25; i++)
                    {
                        token = strtok(NULL, ",");
                        double robot_q = atof(token);
                        robotModel[i].q = robot_q + ori_robotModel[i].q;
                        //cout << "[out] " << robot_q << endl;
                    }
                    #if ROSPUB
                    ikidRobotDynaPosPubSpecialGait();
