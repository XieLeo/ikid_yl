...
#include "localization_pkg/yuvClasses.h"
...
void FindField::findField(FrameGrabber &CamFrm)
{
	...
	//清零操作：使用 memset 函数将 CamFrm.FieldBoundary 和 CamFrm.FieldMask 初始化为零，表示初始时没有任何场地信息。
	...
	// 重置当前场地数据：CamFrm.currentFieldData.reset() 用于清除之前的场地信息，准备新的数据处理。
	...
	{ //<- Left to right.
		...
		for (int y = (SUB_SAMPLING_HEIGHT - 1); y >= 0; y--)
		{ //<- Bottom-up.
			...
			if (CamFrm.cameraMask[pixelpos] != 0)
			{
				...
				else
				{
					...
					if (blank >= MAX_BLANKS)
					{				 //<- N continuous blanks found in a column.
	...
	cv::Mat element = cv::getStructuringElement(0, cv::Size(0 + 5, 0 + 5), cv::Point(0, 0));//edit5.3
	// cv::erode(srcerode, srcerode, element);//edit5.3
	// cv::dilate(srcerode, srcerode, element);//edit5.3
	...
	cv::imshow("FieldBoundary", cv::Mat(SUB_SAMPLING_HEIGHT, SUB_SAMPLING_WIDTH, CV_8U, CamFrm.FieldBoundary));
	// cv::imshow("FieldMask", cv::Mat(SUB_SAMPLING_HEIGHT, SUB_SAMPLING_WIDTH, CV_8U, CamFrm.FieldMask));
	cv::waitKey(5);
