#pragma once
#include "localization_pkg/math.hpp"
#include <iostream>
#include <vector>
...
#define PARTICLE_DEBUGGING 0
    ...
    public:
        ...
        ParticleFilter(InitializerT *particleInitializer,
                       MotionModelT *motionModel,
                       ObservationModelT *observationModel,
            ...
            : m_particleInitializer(particleInitializer), m_motionModel(motionModel), m_observationModel(observationModel), m_minNumParticles(minNumParticles), m_maxNumParticles(maxNumParticles), m_currNumParticles(maxNumParticles), m_prevNumParticles(0), m_hasResampled(false), m_minEffectiveSampleSizeRatio(1.f), m_slowAvgLikelihoodLearningRate(slowAvgLikelihoodLearningRate), m_fastAvgLikelihoodLearningRate(fastAvgLikelihoodLearningRate)
        {

            // std::cout << "maxNumParticles " << maxNumParticles << ":" << std::endl;
            ...
            // * 粒子初始化 * //
            //m_particles有两组，p是每一组里的粒子数量，设置权重为0
        ...
        virtual void
        establishDataAssociation(ParticleT &particle, std::vector<T_obs> &observations) {}
        ...
        {
            std::cout << "importance  no" << std::endl;
        ...
        {
            double avgLikelihood = 0;
            // particals from last cycle
            //其中m_currParticleSetIdx = 0
            // std::cout << "m_currNumParticles " << m_currNumParticles << ":" << std::endl;
            ...
            for (unsigned int p = 0; p < m_currNumParticles; p++)
            {
                ...
                // data association between current observations and last resampled particals
                establishDataAssociation(particle, observations);
                for (unsigned int o = 0; o < observations.size(); o++)
                {
                    T_obs &observation = observations[o];
        ...
        resample()
        {
            ...
            ParticleSetT &particles = m_particles[m_currParticleSetIdx];
            ParticleSetT &tmp_particles = m_particles[1 - m_currParticleSetIdx];
    ...
    protected:
        ...
        ParticleSetT m_particles[2];
        unsigned int m_currParticleSetIdx;

        InitializerT *m_particleInitializer;
        MotionModelT *m_motionModel;
        ObservationModelT *m_observationModel;
