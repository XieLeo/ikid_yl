...
#include <localization_pkg/Detections.h> //ros intertface from "soccer_vision" pkg

// #include <config_server/parameter.h>

#include <localization_pkg/CompassHeading.h> //ros intertface from "robotcontrol" pkg
...
public:
	...
	{
		lik.minLikelihood = m_alphauniform_X_max2rangeinv_X_pi2inv + m_alpha_exp * exp(-lik.min_mahal_dist);
		if (lik.mahal_dist > INT_MAX / 2)
			lik.likelihood = lik.minLikelihood;
		else
			lik.likelihood = m_alphauniform_X_max2rangeinv_X_pi2inv + m_alpha_exp * exp(-lik.mahal_dist);
	}
...
public:
	...
	void drawParticles(cv::Mat &image, double scaleX, double scaleY, const cv::Point2d &center);
...
private:
	...
	void handleDetections(const localization_pkg::DetectionsConstPtr &msg);
	void determineMatch(
	...
	void handleCompass(const localization_pkg::CompassHeadingConstPtr &compass);
	...
	localization_pkg::DetectionsConstPtr m_vision_detections;
	ros::Time m_odomTimestamp;

	ros::Publisher m_pub_all_poses;
	ros::Publisher m_pub_pose;
	ros::Subscriber m_sub_initPose;

	ros::Subscriber m_sub_compass;
	localization_pkg::CompassHeadingConstPtr m_compassHeading;
	...
	config_server::Parameter<bool> m_use_odom;
};
