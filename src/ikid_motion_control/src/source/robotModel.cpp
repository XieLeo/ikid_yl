...
double state_space_Com[2][3] = {{0,0,0},{0,-0.2,-0.2}}; // ���ٶȺͼ��ٶ����ú��ʵĳ�ֵ������ֹ��������
double sum_e[2] = { 0,0 };
const int N_preview = 3 * step_basic_frame + 2 * ds_frame;
// ������
// double ks = 550.063976230535;
// double kx[3] = { 12594.6882596096,	2498.07064206629,	62.1693869335029 };
// double zmp_weight_f[N_preview] = {
// 	-550.063976230535	,-698.975798523916	,-829.366699041033	,-887.973957223364	,-879.063492278202	,
// 	-826.514142155273	,-754.078250707435	,-678.196035097405	,-607.556965334254	,-545.265678998595	,
// 	-491.316025252014	,-444.438993194582	,-403.168640616971	,-366.302175264945	,-332.999605280945	,
// 	-302.720580703234	,-275.119563971675	,-249.956270861454	,-227.037782162488	,-206.188955862243	,
// 	-187.241516235458	,-170.032767473597	,-154.407784497439	,-140.221828765799	,-127.341765325936	,
// 	-115.646363156224	,-105.025798671946	,-95.3807447219992	,-86.6213337788060	,-78.6661612914321	,
// 	-71.4413985759650	,-64.8800266098183	,-58.9211757194752	,-53.5095494017827	,-48.5949131255138	,
// 	-44.1316343506476	,-40.0782649244667	,-36.3971604017590	,-33.0541327482606	,-30.0181337743739	,
// 	-27.2609669734441	,-24.7570255353330	,-22.4830543548875	,-20.4179339315241	,-18.5424841744123	,
// 	-16.8392862753322	,-15.2925209704069	,-13.8878216684947	,-12.6121410699224	,-11.4536300312636	,
// 	-10.4015275498986	,-9.44606084746525	,-8.57835462576068	,-7.79034865374208	,-7.07472292130324	,
// 	-6.42482966541982	,-5.83463163782198	,-5.29864604115981	,-4.81189361318880	,-4.36985238627506	,
// 	-3.96841569292372	,-3.60385402745722	,-3.27278040977305	,-2.97211892962328	,-2.69907617938351	,
// 	-2.45111531009186	,-2.22593246988881	,-2.02143540610354	,-1.83572403231649	,-1.66707277996870	,
// 	-1.51391457065428	,-1.37482626027780	,-1.24851541992244	,-1.13380833068413	,-1.02963908099739	,
// 	-0.935039665213792	,-0.849130991490131	,-0.771114715485168	,-0.700265824031475	,-0.635925899911796	,
// 	-0.577497005193372	,-0.524436126316903	,-0.476250129352748	,-0.432491178574003	,-0.392752575798155
// };
...
void ikidRobotDynaPosPubInit(ros::NodeHandle& n_){
    ...
    // sub_ = n_.subscribe("/subscribed_topic", 1, &SubscribeAndPublish::callback, this);  //ע�������ƽʱʹ�ûص�������һ���ˡ�
...
void clearTxt()
{
	FILE* fp;
	char ch[200];
	char filename[] = "/home/nvidia/ikid_ws2/MOS2023.txt";
	fp = fopen(filename, "w");
	if (fp == NULL)
	{
		exit(0);
...
void waistPosition_com(double r,double p,double y, int current_frame_count) {   // ����ȫ�����ĵ�λ����ǣ��������λ��
...
void startTrajPlan(){
	...
	dFootSupportPhaseStartTrajPlan(theta,theta,theta);
	support_ZMP[0] = pn[0];
	support_ZMP[1] = pn[1];
	...
	if (isLeft) {
		...
		for (int i = 0; i < step_basic_frame; i++)
		{
			...
			if(walk_with_ball){
				...
				//  x = CP_norm + -(i + 1) * 2 * CP_norm / step_basic_frame; // sin����
			...
			#if PID_AMEND
			robotModel[RIGHT_ANKLE_SIDE_SWING].q -= delta_roll;   // ��������ΪҪ�����淴��������������̬
...
void leftTrajPlan(){
	...
	if(stop_walk_flag){
		state_space_Com[0][1] += state_space_Com[0][1]*0.5; //΢����Ϊ���ٴ��ȶ�����
		state_space_Com[0][2] += state_space_Com[0][2]*0.5;
		state_space_Com[1][1] += state_space_Com[1][1]*0.5;
		state_space_Com[1][2] += state_space_Com[1][2]*0.5;
		startTrajPlan();
	}
	double src_sy = sy;
	sx = 0;
	...
	while (1)
	{
		...
		if (fgets(c,sizeof(c),fptr) != NULL)
		{
			...
			if (temp_id == id){
				...
				double before_gait_frame_data[26] = {0};  // ʹ��before������Ϊ�˱��ֶ��������ᣬ���ֱ���õ�ǰ�����˵Ĺؽڽǣ���������ɶٸ�
				...
				while (fgets(c,sizeof(c),fptr) != NULL)
				{
                	...
                	if(token != NULL){
						...
						token = strtok(NULL, ","); // ��ȡ��ǰ֡����һ֮֡��Ĳ�֡��
...
void judgeFall(){
	...
	if(data_pitch < 110 && data_pitch > 70){
		double temp_sx = sx;
		sx = 0;   // ִ�е�������ʱ��һ��Ҫ�Ȱ�ǰ��������Ϊ0��һ����Ҫ��Ȼ��λʱ֮ǰ�����ZMP������Ѿ�����Դ����ڽŵװ���
		trajPlan();
		ros::param::set("stop_walk_flag",true);
...
void FallUpInitPos(){
	...
	state_space_Com[0][1] += -state_space_Com[0][1]*0.25; //΢����Ϊ���ٴ��ȶ�����
...
void writeImuData(){
	...
	FILE* fp = NULL;
	char ch[100];
	char filename[] = "/home/nvidia/ikid_ws2/imu_rpy_data.txt";
	fp = fopen(filename, "a");
	if(fp == NULL)
	{
		exit(0);
...
void writeZmpData(double zmp_data[2][N_preview], double z_d_x, double z_d_y, double z_p_x, double z_p_y,double z_f_x, double z_f_y){
	...
	FILE* fp = NULL;
	char ch[100];
	char filename[] = "/home/nvidia/ikid_ws2/zmpxydesire_zmpxyfact_data.txt";
	fp = fopen(filename, "a");
	if(fp == NULL)
	{
		exit(0);
...
void quinticPolyInterFour(double A[6][4], double B[6][4], double s){
	...
	//double x1[5] = {s/2, 3/10.0*s, 1/10.0*s, -3/10.0*s, -s/2}; // x�����λ��
	double x1[5] = {s/2, 3/10.0*s, 1/10.0*s, -s/2*1.5, -s/2}; // x�����λ��
	double x2[5] = {0, (x1[1]-x1[0])/(t[1]-t[0]), (x1[2]-x1[1])/(t[2]-t[1]), 0.3*(x1[3]-x1[2])/(t[3]-t[2]), 0 }; // x������ٶ�
	double x3[5] = {0, (x2[1]-x2[0])/(t[1]-t[0]), (x2[2]-x2[1])/(t[2]-t[1]), (x2[3]-x2[2])/(t[3]-t[2]), 0 }; // x����ļ��ٶ�
	double z1[5] = {0, fh, 3/5.0*fh, 2/5.0*fh, 0}; // z�����λ��
	double z2[5] = {0, 0, (z1[2]-z1[1])/(t[2]-t[1]), (z1[3]-z1[2])/(t[3]-t[2]), 0 };  // z������ٶ�
	double z3[5] = {0, 0, (z2[2]-z2[1])/(t[2]-t[1]), (z2[3]-z2[2])/(t[3]-t[2]), 0 };  // z����ļ��ٶ�
...
void quinticPolyInterTwo(double A[6][4], double B[6][4], double s){
	...
	double x1[5] = {s/2, 0, -s/2}; // x�����λ��
	double x2[5] = {0, (x1[1]-x1[0])/(t[1]-t[0]), 0 }; // x������ٶ�
	double x3[5] = {0, (x2[1]-x2[0])/(t[1]-t[0]), 0 }; // x����ļ��ٶ�
	double z1[5] = {0, fh, 0}; // z�����λ��
	double z2[5] = {0, 0, 0 };  // z������ٶ�
	double z3[5] = {0, 0, 0 };  // z����ļ��ٶ�
...
void threeSplineInter(double spline_A[8][2], double s){
	...
	double x[3] = {s/2, 0, -s/2}; // x�����λ��
	double z[3] = {0, fh, 0}; // z�����λ��
