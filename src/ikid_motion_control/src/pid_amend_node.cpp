...
void doImuMsg(const sensor_msgs::Imu::ConstPtr& imuMsg_p){
	...
	if(fout.fail()){
        fout.open("/home/nvidia/ikid_ws2/imubuffer.txt", std::ios::app);
        fout.close();
        fout.open("/home/nvidia/ikid_ws2/imubuffer.txt", std::ios::out);
    }
    fout.seekp(0);
    fout << roll << ' ' << pitch << ' ' << yaw;
    // printf("%f, %f\n",roll, pitch);
...
int main(int argc, char *argv[])
{
    ...
    ros::NodeHandle n;
    // imu测试话题数据
    ros::Subscriber suber = n.subscribe<sensor_msgs::Imu>("/ikid/imu/data",10,doImuMsg);
	fout.open("/home/nvidia/ikid_ws2/imubuffer.txt", std::ios::out);
