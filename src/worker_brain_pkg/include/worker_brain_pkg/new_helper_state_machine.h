    ...
    */
    ...
    std::vector<float> cal_object_pos(int object_calss) {//class robot = 1, football = 0
        ...
        if(object_calss == 0) {// football
            ...
            // 3D 特征点世界坐标，与像素坐标对应，单位是cm, 构造image_point3d
            // std::vector<Point3d> model_points;
            model_points.push_back(Point3d(-6.95f*4.1, +6.95f*4.1, 0)); 
            model_points.push_back(Point3d(+6.95f*4.1, -6.95f*4.1, 0));
            model_points.push_back(Point3d(+6.95f*4.1, +6.95f*4.1, 0));
            model_points.push_back(Point3d(0, 0, +6.95f*4.1));
            model_points.push_back(Point3d(-3.4750f*4.1, 0, +6.018877f*4.1));
            model_points.push_back(Point3d(0, +5.2125f*4.1, +4.596993f*4.1));
            // model_points.push_back(Point3d(-3.475f, +3.475f, +4.914392f));
            // model_points.push_back(Point3d(0, +3.475f, +6.018877f));
            // model_points.push_back(Point3d(+3.475f, +3.475f, +4.914392f));
            // model_points.push_back(Point3d(+3.475f, 0, +6.018877f));
        ...
        // cal object distance
        float distance = abs(cal_distance(image_points, model_points));
        // ---zhaoyu---
        // cal object pos
        ...
        if(object_calss == 0) {// football
            res[0] = 0;
            res[1] = 6.95;
            res[2] = sqrt(pow(distance, 2) - pow((57-6.95), 2));
        }else if(object_calss == 1){// robot
            float cd_distance = sqrt(pow(distance, 2) - pow(28.5, 2));
            cout << "pnp robot:"<<distance << endl;
            res[0] = cd_distance * sin(angleToRadian(football_angle - robot_angle));
            res[1] = 28.5;
            res[2] = cd_distance * cos(angleToRadian(football_angle - robot_angle));
            cout << "angle" << football_angle - robot_angle << endl;
        }
    ...
    State getNextStateByEnvCurState() {
        switch (cur_state) {
            ...
            case State::FollowFootball:
            {
                ...
                if(cur_env_data->football_xyxy.size() != 0 && football_lost_frames_nums < 6){
                ...
                central_point_y < FOLLOW_FPOINT_EDGE_DOWN && central_point_y > FOLLOW_FPOINT_EDGE_UP) {
                    ...
                    cout << "football_pos: x:" << football_3d_pos[0] << " y:" << football_3d_pos[1] << " z:" << football_3d_pos[2] << endl;
            ...
            case State::FindRobot:
                ...
                // 如果找球过程中发现了球且大于FOUND_FOOTBALL_TRIGGER帧
                if (cur_env_data -> robot_xyxy.size() != 0 && robot_sustain_frames_nums >= 3) {
                    // 头部相关参数归位，停止继续转动摄像头
                    std::cout << "[" << frame << "]连续发现Robot，退出找Robot状态，改变状态为跟Robot" << std::endl;
                ...
                if(frame >= next_find_robot_state_frame) {
                    ...
                    next_find_robot_state_frame = frame + FIND_FOOTBALL_STATE_FRAME_INTERVAL;
                    std::cout << "[" << frame << "]找球模式下没有找到Robot，继续找Robot" << std::endl;
                    return State::FindRobot;
                }
                return State::FindRobot;
                break;
            ...
            {
                int central_point_x, central_point_y;
                if(cur_env_data->robot_xyxy.size() != 0 && robot_lost_frames_nums < 15){
                ...
                central_point_y < FOLLOW_FPOINT_EDGE_DOWN && central_point_y > FOLLOW_FPOINT_EDGE_UP) {
                    ...
                    cout << "robot_pos: x:" << robot_3d_pos[0] << " y:" << robot_3d_pos[1] << " z:" << robot_3d_pos[2] << endl;
                    return State::Move;
                }else {
            ...
            case State::Move:
                {
                    ...
                    if(football_robot_distance < 10) {
                        ...
                        cout << "send move data var_theta = " << data.var_theta << endl;
                        pauseNFrame(100);
                        return State::FindFootball;
                    }
                }
            ...
            case State::Kick:
                ...
                // udp发送数据给worker
                sender.SendData(data);
                cout << "send kick data kick_leg = " << data.kick_leg << endl;
                pauseNFrame(100);
                return State::FindFootball;
                break;
            default:
