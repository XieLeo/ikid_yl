...
class WorldObjectType(Enum):
    Type_Circle = 1         # Center circle
    Type_Goal = 2           # Goal (center position)
    Type_GoalPost = 3       # A single goal post
    Type_XMarker = 4        # One of the two penalty markers
    Type_FieldLine = 5      # Field line
    Type_LineXingT = 6      # T-crossing of two lines
    Type_LineXingX = 7      # X-crossing of two lines
    Type_LineXingL = 8      # L-crossing of two lines
...
def image_points_callback(msg):
    ...
    for category, markers, obj_type in [
        ("ball", msg.ball_xyxy, WorldObjectType.Type_LineXingL),
